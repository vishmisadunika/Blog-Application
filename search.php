...
<div class="page-header animate-fade-in" style="text-align: center; margin: 2rem 0;">
    <h1 class="script-font" style="font-size: 2.5rem; color: var(--primary);">
        <?php echo $searchQuery !== '' ? 'Results for "' . escape($searchQuery) . '"' : 'Search articles'; ?>
    </h1>
    <?php if ($searchQuery !== ''): ?>
        <p style="color: var(--text-light);"><?php echo count($posts); ?> article<?php echo count($posts) !== 1 ? 's' : ''; ?> found ♡</p>
    <?php endif; ?>
</div>

<?php if ($searchQuery === ''): ?>
    <div class="empty-state animate-fade-in" style="text-align: center; padding: 4rem 1rem;">
        <h2 class="script-font" style="color: var(--primary); font-size: 2rem;">What are you looking for? ♡</h2>
        <p style="color: var(--text-light);">Use the search bar above to find articles by title or content.</p>
    </div>
<?php elseif (empty($posts)): ?>
    <div class="empty-state animate-fade-in" style="text-align: center; padding: 4rem 1rem;">
        <h2 class="script-font" style="color: var(--primary); font-size: 2rem;">No matches found ♡</h2>
        <p style="color: var(--text-light); margin-bottom: 1.5rem;">Try a different keyword, or browse all articles instead.</p>
        <a href="index.php?view=all" class="btn btn-primary">Browse all articles</a>
    </div>
<?php else: ?>
    <div class="blog-grid">
