...
if ($postId > 0) {
    ...
    $stmt = $conn->prepare("
        SELECT id, user_id, title, content, excerpt, topic_id, cover_image
        FROM blogPost
        WHERE id = ?
    ");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("
            UPDATE blogPost
            SET title = ?, content = ?, excerpt = ?, topic_id = ?, cover_image = ?, reading_time = ?, updated_at = CURRENT_TIMESTAMP
            WHERE id = ? AND user_id = ?
        ");
        $stmt->bind_param('sssisiii', $title, $content, $excerpt, $topic_id, $cover_image, $reading_time, $postId, $postUserId);
...
<div class="editor-page">
    <div class="page-header animate-fade-in" style="margin-bottom: 2rem;">
        <h1 class="page-title script-font" style="font-size: 2.5rem; color: var(--primary);">Edit your story ♡</h1>
        <p class="page-subtitle" style="color: var(--text-light);">Refining thoughts is a beautiful process.</p>
    </div>
    ...
    <form method="POST" action="edit.php?id=<?php echo $postId; ?>" class="editor-form" id="blog-form" enctype="multipart/form-data">
        <div class="editor-layout" style="display: grid; grid-template-columns: 1fr 300px; gap: 2rem;">
            <div class="editor-main">
                ...
                <div class="editor-footer" style="display: flex; justify-content: space-between; align-items: center; margin-top: 1.5rem; padding-top: 1rem; border-top: 1px solid var(--border);">
                    ...
                    <div class="form-actions" style="display: flex; gap: 0.5rem; align-items: center;">
                        ...
                        <a href="delete.php?id=<?php echo $postId; ?>" class="btn btn-ghost" style="color: var(--danger); margin-left: 1rem;" onclick="return confirm('Are you sure you want to delete this post?');">Delete</a>
                    </div>
                </div>
            </div>
            ...
            <div class="editor-sidebar">
                ...
                <div class="form-group" style="margin-bottom: 2rem;">
                    <label style="display: block; margin-bottom: 0.5rem; color: var(--primary-dark); font-weight: 500;">Cover image</label>
                    <div class="cover-upload" id="cover-upload-zone" style="border: 2px dashed var(--border); border-radius: 8px; padding: 2rem 1rem; text-align: center; cursor: pointer; background: var(--bg-card); transition: all 0.2s; <?php echo !empty($cover_image) ? 'display: none;' : ''; ?>">
                    ...
                    <div class="cover-preview" id="cover-preview" style="<?php echo empty($cover_image) ? 'display: none;' : ''; ?> position: relative; border-radius: 8px; overflow: hidden; margin-top: 0.5rem;">
                        <img src="<?php echo escape($cover_image); ?>" alt="Cover preview" id="cover-preview-img" style="width: 100%; height: auto; display: block;">
